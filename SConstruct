...
pretty_dep_names = {
    ...
    'java':'Java JNI Library | configure with JAVA_AWT_INCLUDE_PATH, JAVA_INCLUDE_PATH, JAVA_INCLUDE_PATH2, JAVA_AWT_LIB & JAVA_JVM_LIB | more info: https://github.com/mapnik/mapnik/wiki/Java',
    'geowave':'GeoWave database library | configure with GEOWAVE_LIBS, GEOWAVE_INCLUDES, GEOWAVE_JACE_RUNTIME_JAR & GEOWAVE_RUNTIME_JAR | more info: https://github.com/mapnik/mapnik/wiki/GeoWave',
...
opts.AddVariables(
    ...
    PathVariable('JAVA_AWT_INCLUDE_PATH', 'Search path for Java Abstract Windowing Toolkit include files', '', PathVariable.PathAccept),
    ...
    PathVariable('JAVA_AWT_LIB', 'Search path for Java Abstract Windowing Toolkit library files', '', PathVariable.PathAccept),
    ...
    PathVariable('GEOWAVE_JACE_RUNTIME_JAR', 'Full path to GeoWave Jace Runtime Jar', '', PathVariable.PathAccept),
...
def EnableJava(env):
    ...
    if (not JAVA_ENABLED) and (env.get('JAVA_AWT_INCLUDE_PATH') or env.get('JAVA_INCLUDE_PATH') or env.get('JAVA_INCLUDE_PATH2') or env.get('JAVA_AWT_LIB') or env.get('JAVA_JVM_LIB')):
        backup = env.Clone().Dictionary()
        incpath = [env.get('JAVA_AWT_INCLUDE_PATH'), env.get('JAVA_INCLUDE_PATH'), env.get('JAVA_INCLUDE_PATH2')]
        libpath = [env.get('JAVA_AWT_LIB'), env.get('JAVA_JVM_LIB')]
        env.PrependUnique(CPPPATH = copy(incpath))
        env.PrependUnique(LIBPATH = copy(libpath))
        if (not conf.CheckLibWithHeader(details['lib'][0], details['inc'], details['lang'])) or (not conf.CheckLibWithHeader(details['lib'][1], details['inc'], details['lang'])):
            env.Replace(**backup)
        ...
        if env['HOST']:
            ...
            for plugin in env['REQUESTED_PLUGINS']:
                ...
                if plugin == 'gdal':
                    ...
                    EnableJava(env)
                    if env.get('GEOWAVE_LIBS') or env.get('GEOWAVE_INCLUDES') or env.get('GEOWAVE_RUNTIME_JAR') or env.get('GEOWAVE_JACE_RUNTIME_JAR'):
                        backup = env.Clone().Dictionary()
                        incpath = env['%s_INCLUDES' % details['path']]
                        libpath = env['%s_LIBS' % details['path']]
                        includes = [os.path.relpath(y, incpath) for x in os.walk(incpath) for y in glob(os.path.join(x[0], '*.h'))]
                        env.PrependUnique(CPPPATH = copy(incpath))
                        env.PrependUnique(LIBPATH = copy(libpath))
                        try:
                            gwVer = re.search('(?:geowave-deploy-)(.*)(?:-jace.jar)',env.get('GEOWAVE_RUNTIME_JAR')).group(1)
                            jcVer = re.search('(?:jace-core-runtime-)(.*)(?:.jar)',env.get('GEOWAVE_JACE_RUNTIME_JAR')).group(1)
                        except:
                            gwVer = ''
                            jcVer = ''
                        if (not gwVer) or (jcVer != '1.2.22') or (not conf.CheckLibWithHeader(details['lib'], includes, details['lang'])):
                            env.Replace(**backup)
                            env['SKIPPED_DEPS'].append('geowave')
                        else:
                            color_print(4,'Found GeoWave lib version... %s' % gwVer)
